...
#
# Usage: bash analysis/tardis_options/run_fetch.sh
#
# Monitor: tail -f analysis/tardis_options/data/fetch_log.txt

set -e

REPO_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
LOG="$REPO_DIR/analysis/tardis_options/data/fetch_log.txt"
VENV="$REPO_DIR/.venv/bin/activate"

mkdir -p "$REPO_DIR/analysis/tardis_options/data"
...
nohup caffeinate -i python -m analysis.tardis_options.fetch \
  --from 2026-03-10 \
  --to 2026-03-23 \
  >> "$LOG" 2>&1 &
